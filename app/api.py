...
class ValorantAPI:
        ...
        if headers is None:
            if self.cached_headers is None:
                raise RuntimeWarning(
                    'No auth headers found! Please call get_auth with cache_headers=True.')
            else:
    ...
    def get_region_from_local_log() -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f'Error occurred while reading VALORANT logs for region: {str(e)}')
    ...
    def get_cookies(self, force: bool = False) -> Optional[Cookies]:
        ...
        try:
            with open(ValorantAPI.get_cookies_path()) as cookie_file:
                cookies = yaml.safe_load(cookie_file)[
    ...
    def _auth_with_cookies(self, force: bool, cache_headers: bool):
        try:
            ...
            initial_auth = session.post(
                ValorantAPI.INITIAL_AUTH_URL, json=params, headers=headers, allow_redirects=False)
            if initial_auth.status_code != 200:
                raise Exception(
                    "Unable to perform initial auth. Falling back to lockfile...")
            redirect = session.get(
                ValorantAPI.REAUTH_URL, headers=headers, allow_redirects=False)

            redirect_url: str = redirect.headers['Location']
            res_dict = dict(parse.parse_qsl(
                parse.urlsplit(redirect_url).fragment))
            if "access_token" not in res_dict:
                raise Exception(
                    "No access_token found for cookie re-auth. Are you logged into the Riot Client? Falling back to lockfile...")
    ...
    def get(self, url: str, params: Optional[Dict] = None, headers: Optional[Headers] = None) -> Dict:
        response = r.get(url, headers=self._fill_headers(
    ...
    def get_season_by_id(self, season_id: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        episode_info = self.get_valorant_api(
            f"/v1/seasons/{season_info['parentUuid']}")
    ...
    def get_match_history_info(self, puuid: str, start_index: int, end_index: int, queue: Optional[str] = None):
        def request(new_start: int, new_end: int):
            data = {'startIndex': new_start, 'endIndex': new_end,
    ...
    #def get_all_match_history(self, puuid: str, queue: Optional[str] = None):
        #first_20_matches = self.get_match_history_info(
            #puuid, start_index=0, end_index=20, queue=queue)
    ...
    def get_all_match_history(self, puuid: str, queue: Optional[str] = None):
        first_20_matches = self.get_match_history_info(
            puuid, start_index=0, end_index=50, queue=queue)
